(() => {
  ...
  function stationLabel(type = curController) {
    const frequency = controllerFrequency(type);
    return type ? `${controllerNames[type]}${frequency ? ' · ' + frequency : ''}` : 'No controller selected';
  }
  ...
  function renderFreqs() {
    ...
    controllerOrder.forEach(type => {
      const frequency = controllerFrequency(type);
      const available = type === 'ATIS' ? Boolean(curIcao) : Boolean(frequency);
      const button = document.createElement('button'); button.type = 'button';
      button.className = 'fbt' + (type === curController ? ' active' : '');
      button.disabled = !available;
      button.textContent = `${controllerNames[type]}${frequency ? ' ' + frequency : type === 'ATIS' && curIcao ? '' : ' —'}`;
      button.addEventListener('click', () => selectController(type, true));
  ...
  async function persistController(announce = false) {
    const payload = {
      simbrief_id: simbrief.value.trim(), callsign: callsign.value.trim(), gate: gate.value.trim(), arrival_gate: arrivalGate.value.trim(),
      airport: curIcao, controller_type: curController, controller_frequency: controllerFrequency(curController),
    };
  ...
  async function selectController(type, persist) {
    ...
    curController = type;
    const frequency = controllerFrequency(type);
    freqBadge.innerHTML = type === 'ATIS' ? 'ATIS' : `${controllerNames[type]} <b>${frequency}</b>`;
    controllerStatus.textContent = `Active voice station: ${stationLabel()}. AeroSpeak will answer only as this controller.`;
    renderFreqs();
    if (persist) {
      try { await persistController(false); }
      catch (error) { controllerStatus.textContent = error.message; return; }
    }
  ...
  async function loadFreqs(icao, preferredType = '', persistSelection = false) {
    ...
    try {
      const response = await fetch('/api/frequencies/' + encodeURIComponent(icao));
      if (response.ok) curFreqs = (await response.json()).freqs || {};
    } catch (_) {}
    const chosen = (preferredType && (preferredType === 'ATIS' || curFreqs[preferredType])) ? preferredType :
      ['CLD', 'GND', 'TWR', 'APP', 'DEP', 'ATIS'].find(type => type === 'ATIS' || curFreqs[type]);
    if (chosen) await selectController(chosen, persistSelection);
    else { curController = ''; freqBadge.textContent = 'FREQ —'; controllerStatus.textContent = `No controller frequencies are listed for ${icao}.`; renderFreqs(); }
  }

  async function ensureMic() {
    if (stream) return;
    stream = await navigator.mediaDevices.getUserMedia({ audio: { echoCancellation: true, noiseSuppression: true } });
    ctx = new (window.AudioContext || window.webkitAudioContext)(); analyser = ctx.createAnalyser(); analyser.fftSize = 512;
    ctx.createMediaStreamSource(stream).connect(analyser);
    const samples = new Uint8Array(analyser.frequencyBinCount);
    const tick = () => {
      if (recording && analyser) {
        analyser.getByteTimeDomainData(samples); let total = 0;
        for (let i = 0; i < samples.length; i++) { const value = (samples[i] - 128) / 128; total += value * value; }
  ...
  function unlockAudio() {
    try { const silent = new Audio('data:audio/wav;base64,UklGRiQAAABXQVZFZm10IBAAAAABAAEARKwAAIhYAQACABAAZGF0YQAAAAA='); silent.play().then(() => silent.pause()).catch(() => {}); } catch (_) {}
  }

  async function startRecording() {
    if (!curIcao || !curController) { log('sys', 'Open Settings and select an airport plus the controller frequency you are calling.'); setState('station required'); return; }
    ...
    recorder.start(); mediaRecorder = recorder; recording = true; rx.classList.add('live'); ptt.classList.add('hold'); setState('TX ' + stationLabel());
  }
  ...
  async function sendAudio(blob) {
    ...
    const form = new FormData(); form.append('audio', blob, 'transmission.webm'); setState('transmitting…');
    try {
      const response = await fetch('/api/chat', { method: 'POST', body: form }); const data = await response.json();
      if (!response.ok) { log('sys', data.detail || data.error || 'Radio request failed.'); setState('error'); setTimeout(() => setState('ready'), 1600); return; }
      if (data.text) log('atc', data.text); if (data.audio) playAudio(data.audio);
      if (data.context_refreshed) saved.textContent = `Live ${curIcao} METAR and ATIS refreshed.`;
      setState('ready');
  ...
  async function playAtis() {
    ...
    try {
      ...
      if (!response.ok) { log('sys', 'ATIS unavailable: ' + (data.error || 'unknown error')); return; }
      log('atc', data.atis); if (data.audio) playAudio(data.audio);
  ...
  async function searchAirports(query) {
    ...
    try {
      const response = await fetch('/api/airports?q=' + encodeURIComponent(query)); const data = await response.json(); apresults.textContent = '';
      (data.airports || []).forEach(airport => {
        const row = document.createElement('div'); row.className = 'ap'; const left = document.createElement('span'); const code = document.createElement('b'); code.textContent = airport.icao;
        left.append(code, document.createTextNode(` ${airport.name || ''} · ${airport.city || ''} ${airport.country || ''}`)); row.appendChild(left);
        row.addEventListener('click', async () => { apsearch.value = airport.icao; apresults.classList.remove('open'); await loadFreqs(airport.icao, '', true); });
  ...
  async function saveSettings(announce = true) {
    const data = await persistController(announce); syncPlan.disabled = !simbrief.value.trim();
    renderPlanStatus(data.flight_plan || {}, data.validation, data.station || {});
  }

  async function syncFlightPlan() {
    syncPlan.disabled = true; syncPlan.textContent = 'Syncing Flight Plan…'; planstatus.textContent = 'Retrieving your latest SimBrief briefing…';
    try {
      ...
      renderPlanStatus(plan, data.validation, data.station || {}); saved.textContent = 'Flight plan synchronized. Select the controller frequency you are calling.';
  ...
  async function validateFlightPlan() {
    ...
    try {
      ...
      renderPlanStatus(data.flight_plan, data.validation, data.station || {}); saved.textContent = data.context_refreshed ? 'Live airport details refreshed.' : 'No active airport to refresh.';
  ...
  async function loadSettings() {
    try {
      ...
      simbrief.value = data.simbrief_id || ''; callsign.value = data.callsign || ''; gate.value = data.gate || ''; arrivalGate.value = data.arrival_gate || '';
      accessBlock.hidden = !(data.access_required && !data.authorized); syncPlan.disabled = !simbrief.value.trim();
      const airport = data.airport || data.flight_plan?.origin || ''; if (airport) { apsearch.value = airport; await loadFreqs(airport, data.controller_type || '', false); }
      renderPlanStatus(data.flight_plan || {}, data.validation, data.station || {});
      if (!airport) controllerStatus.textContent = 'Select an airport, then choose the controller frequency you are calling.';
  ...
  ptt.addEventListener('pointerup', event => { event.preventDefault(); stopRecording(); }); ptt.addEventListener('pointerleave', stopRecording);
  ...
  gear.addEventListener('click', () => settings.classList.toggle('open')); closeBtn.addEventListener('click', () => settings.classList.remove('open'));
  saveBtn.addEventListener('click', async () => { saveBtn.disabled = true; try { await saveSettings(true); } catch (error) { saved.textContent = error.message; } finally { saveBtn.disabled = false; } });
  syncPlan.addEventListener('click', syncFlightPlan); validatePlan.addEventListener('click', validateFlightPlan);
  unlock.addEventListener('click', async () => {
    unlock.disabled = true;
    try {
      const response = await fetch('/api/access', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ code: accessCode.value }) }); const data = await response.json();
      if (!response.ok) throw new Error(data.detail || 'Access was not accepted.'); accessCode.value = ''; accessBlock.hidden = true; saved.textContent = 'Live services unlocked for this browser session.';
    } catch (error) { saved.textContent = error.message; } finally { unlock.disabled = false; }
  });
  loadSettings();
})();
